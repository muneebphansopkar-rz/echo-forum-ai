...
> Sourced from `module-briefs/echo-forum/BRIEF.md` on 2026-04-18.
> If this conflicts with the module brief, update both in lockstep.
...
7. **Search** — simple Postgres full-text search across posts in the
   caller's community only.
...
Authenticated via the main SKEP platform (JWT from Keycloak), scoped to a
single community at a time.

### Secondary User

`MODERATOR`s and `ADMIN`s who keep the forum healthy. Community `OWNER`s
who shape the forum's structure (tags, categories, edit windows).

## Goals for This Build

1. **A member can create a post, receive a reply, and upvote it** end-to-end.
2. **Feed sorting (Hot)** works correctly — time-decay + upvote algorithm
   that visibly changes ordering as activity happens.
3. **Moderation works** — a `MODERATOR` can hide a reply, and the reply
   shows as hidden to non-authors while the author still sees the original.
4. **Three domain events published** to the Redis bus:
   `forum.post.created`, `forum.reply.added`, `forum.post.upvoted`.
5. **Search finds posts by content** within a community; cross-community
   leakage is zero.

## What This Is NOT

- **Not a wiki.** Posts are immutable-ish (15-minute edit window, not
  continuous editing).
...
- **Not a file-sharing platform.** Inline images in posts are fine; bulk
  file drop is out of scope.
- **Not a multi-community aggregator.** Strict tenant isolation — schema
  per community, no cross-schema joins.
...
- Single identity across chat, forum, tutor, website — no username
  collision games.
- Notifications go through the shared Campaigns module with
  user-respected preferences.
- Costs less than a paid Circle seat per member.

## Timeline

- **MVP target date:** Hackathon T+72h (T+0 to be confirmed)
...
Bundled with all SKEP tiers. Per-tier limits (posts per day, replies per
day, attachment storage) enforced via `LmsClient.checkLimits`.

---

## Figma

Figma: *(to be provided — placeholder references acceptable for MVP)*
